	...
	*/
	...
	public function display_forums_modify_template_vars($event)
	{
		...
		if ($fs)
		{
			...
			if ($fs_parse_type == self::PARSE_AS_BBCODE)
			{
				//parse BBCode
	...
	public function viewtopic_assign_template_vars_before($event)
	{
		...
		if ($fs_above)
		{
			...
			if ($fs_above_parse_type == self::PARSE_AS_BBCODE)
			{
	...
	public function acp_manage_forums_request_data($event)
	{
		...
		$fs_uid = $fs_bitfield = '';
		$fs_options=7;
		$fs_parse_type = $this->request->variable('forum_sponsor_parse_type', self::PARSE_AS_HTML);
		$fs = $this->parse_text_by_parse_type($fs_parse_type, $fs, $fs_uid, $fs_bitfield, $fs_options);

		$fs_above_parse_type = $this->request->variable('forum_sponsor_above_parse_type', self::PARSE_AS_HTML);
	...
	public function acp_manage_forums_display_form($event)
	{
			...
			if ($fs && $forum_data['forum_sponsor_parse_type'] == self::PARSE_AS_BBCODE)
			{
			...
			if ($fs_above && $forum_data['forum_sponsor_above_parse_type'] == self::PARSE_AS_BBCODE)
			{
			...
			$template_data += array(
				'FORUM_SPONSOR'	=>$fs,
				'FORUM_SPONSOR_ABOVE'	=> $fs_above,
				'S_FORUM_SPONSOR_HTML_CHECKED'	=>  isset($forum_data['forum_sponsor_parse_type']) ? $forum_data['forum_sponsor_parse_type'] == self::PARSE_AS_HTML ? true :false :false,
				'S_FORUM_SPONSOR_ABOVE_HTML_CHECKED'	=> isset( $forum_data['forum_sponsor_above_parse_type']) ? $forum_data['forum_sponsor_above_parse_type'] == self::PARSE_AS_HTML ? true :false :false,
			);
